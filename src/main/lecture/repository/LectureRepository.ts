...
import { LectureSearchType } from '../domain/LectureSearchType';
import { LectureOrderType } from '../domain/LectureOrderType';
...
export class LectureRepository {
  ...
  public async findByPage(
    ...
    connection: PoolConnection,
  ): Promise<Array<LectureListResponseItem>> {
    const queryParams: (string | number)[] = [];
    const query: string = `
    ...
    const [rows]: [RowDataPacket[], FieldPacket[]] = await connection.execute(query, queryParams);
    return rows.map((row: RowDataPacket) => LectureListResponseItem.of(
        row.id,
        row.category,
        row.title,
        row.instructor_name,
