  ...
  describe('create', () => {
    ...
    it.each([2, 51])('[Failure] 강의 제목이 3자 이상 50자 이하가 아니면 예외를 던진다 (강의 제목 길이: %i)', (length: number) => {
      ...
      const {
        ...
        category,
        price,
      } = TestLectureDataFactory.createData({ title: testTitle });
    ...
    it.each([0, 5001])('[Failure] 강의 소개가 1자 이상 5000자 이하가 아니면 예외를 던진다.(강의 소개 길이: %i)', (length: number) => {
      ...
      const {
        ...
        category,
        price,
      } = TestLectureDataFactory.createData({ introduction: testIntroduction });
    ...
    it('[Failure] 강사의 아이디가 1이상이 아니면 예외를 던진다.', () => {
      ...
      const {
        ...
        category,
        price,
      } = TestLectureDataFactory.createData({ instructorId: testInstructorId });
    ...
    it('[Failure] 강의 가격이 0 이상이 아니면 예외를 던진다.', () => {
      ...
      const {
        ...
        category,
        price,
      } = TestLectureDataFactory.createData({ price: testPrice });
